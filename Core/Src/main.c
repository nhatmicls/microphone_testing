...
int main(void)
{
  ...
  while (1)
  {
    ...
    if (newblockdata==1)
    {
    	...
    	lengthbuffer=strlen(UARTbuffer);
    	for (var = 0; var < lengthbuffer; ++var)
    	{
			if(UARTbuffer[var]=='.')
			{
				if (!strcmp(cache,"STA"))
					mode=RECIEVE;
				else if (!strcmp(cache,"END"))
					mode=HALT;
				else if(copy>0)
				{
					if(copy==1)
						adcoutputvalue=strtol(cache, &cache, 10);
					else
						checksum=strtol(cache, &cache, 10);
				}
				var1=0;
				++copy;
			}
			else
				cache[var1++]=UARTbuffer[var];
		}
	}
    switch (mode) {
		case HALT:
